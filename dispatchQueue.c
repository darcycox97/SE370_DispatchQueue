...
dispatch_queue_t *dispatch_queue_create(queue_type_t queue_type)
{
    ...
    queue->dispatcher_thread = dispatcher_thread;
...
{
    // tell the queue not to accept any more tasks
    queue->is_waited_on = 1;

    // free thread pool if queue type is concurrent, otherwise thread pool was never set up
    if (queue->queue_type == CONCURRENT)
    {
        dispatch_queue_thread_t *thread = remove_from_thread_pool(queue->threads);
        while (thread != NULL)
        {
            // only the semaphore and thread were allocated memory separately so destroy them
            free(thread->thread);
            free(thread->thread_semaphore);
            free(thread);
            thread = remove_from_thread_pool(queue->threads);
        }
        free(queue->threads);
    }

    // destroy all tasks remaining in the queue
    task_t *task = remove_from_dispatch_queue(queue);
    while (task != NULL)
    {
        task_destroy(task);
        task = remove_from_dispatch_queue(queue);
    }

    // free all memory that was allocated when queue was created
    free(queue->threads_free_semaphore);
    free(queue->queue_empty_semaphore);
    free(queue->tasks_semaphore);
    free(queue->dispatcher_thread);
    free(queue);
}
...
void dispatch_for(dispatch_queue_t *queue, long n, void (*work)(long))
{
    ...
    for (long i = 0; i < n; i++)
    {
        // create new variable for argument of each created task to avoid concurrency issues with
        // tasks accessing the same memory address as used by the loop counter.
        long parameter = i;

        task_t *task = task_create((void (*)(void *))work, (void *)parameter, "task");
